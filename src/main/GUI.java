		...
		MouseMotionListener, TreeSelectionListener
{
	...
	private final double maxBr = 8.0D;
	private final double lowBr = 0.300000011920929D;
	private final static String folder = "Data";

	private double crtBr = 0.0D;

	public static int speedValue = -300;
	...
	{
		this.menuBar = makeMenu();
		setJMenuBar(this.menuBar);

		this.tree = loadJTree();
		this.left = new JScrollPane(this.tree,
				JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
		this.left.setPreferredSize(new Dimension(200, 650));
		this.left.getViewport().add(this.tree);

		this.image = open(path);

		this.colorPanel = new ColorPanel(this.image);
	...
	private void playPause()
	{
		...
		Thread animation = new Thread(new PlayRunnable(this.colorPanel,	folder));
		if (!isPlaying)
		{
