...
public class DICOMImageReader extends ImageReader {
  ...
  private static final int AE=0x4145, AS=0x4153, AT=0x4154, CS=0x4353, DA=0x4441, DS=0x4453, DT=0x4454, FD=0x4644, FL=0x464C, IS=0x4953, LO=0x4C4F, LT=0x4C54, PN=0x504E, SH=0x5348, SL=0x534C, SS=0x5353, ST=0x5354, TM=0x544D, UI=0x5549, UL=0x554C, US=0x5553, UT=0x5554, OB=0x4F42, OW=0x4F57, SQ=0x5351, UN=0x554E, QQ=0x3F3F, XS=0x5853, OX=0x4F58;
  ...
  private int vr;
  private long metaGroupLength = -1L, dataSetPosition = -1L, imageStartPosition = -1L, metaStart = -1;
  private String transferSyntaxUID;
  private String photometricInterpretation = new String("MONOCHORME2");
  private int bitsStored = 16, bitsAllocated = 16, pixelRepresentation = 1, samplesPerPixel = 1, pixelPaddingValue = -2000;
  ...
  private int redLUTsize, greenLUTsize, blueLUTsize, redLUToffset, greenLUToffset, blueLUToffset, redBits, greenBits, blueBits, lutSize;
  ...
  private void checkIndex(int imageIndex) {
    if(imageIndex >= nImages) throw new IndexOutOfBoundsException("bad index");
  }
  ...
  public boolean validVR(int vr) {
    return(vr == AE || vr == AS || vr == AT || vr == CS || vr == DA || vr == DS || vr == DT || vr == FD || vr == FL || vr == IS || vr == LO || vr == LT || vr == PN || vr == SH || vr == SL || vr == SS || vr == ST || vr == TM || vr == UI || vr == UL || vr == UT || vr == OB || vr == OW || vr == SQ || vr == UN || vr == QQ);
  }
  ...
  public void readHeader() throws IIOException {
    ...
    try {
      ...
      if(signature[0] != (byte)'D' || signature[1] != (byte)'I' || signature[2] != (byte)'C' || signature[3] != (byte)'M') {
        stream.seek(0);
        int tag = getNextTag();
        tag = tag&0xffff0000;
        if(tag != 0x00080000 && tag != 0x08000000) throw new IIOException("Bad DICOM signature!");

        if(tag == 0x08000000) bo = ByteOrder.BIG_ENDIAN; else bo = ByteOrder.LITTLE_ENDIAN;
        int vr = (stream.readByte()<<8) + (stream.readByte());
        if(validVR(vr)) implicit = false; else implicit = true; // fix could get confused if 0x554c happens to be a length as well
        ...
        // has meta information ie part 10 (usually compliant unless you are @%$*#%$#! efilm
        metaStart = stream.getStreamPosition();
        metaGroupLength = 0;

        while(true) {
          int tag = getNextTag();
          if((tag&0xffff0000) != 0x00020000 || stream.getStreamPosition() > 500) break;
          stream.mark();
      ...
      while (!stop) {
        if(transferSyntaxUID != null && stream.getStreamPosition() == dataSetPosition) {
          if(transferSyntaxUID.trim().equals("1.2.840.10008.1.2")) { // little endian implicit
            bo = ByteOrder.LITTLE_ENDIAN;
            stream.setByteOrder(bo);
            implicit = true;
        ...
        int tag = getNextTag();
        long elementLength = getLength(tag);
//System.out.print("tag: " + Integer.toHexString(tag) + " length: " + elementLength + " vr: " + Integer.toHexString(vr));
//System.out.flush();
        Object temp = null;
        if(tag != PIXEL_DATA) {
          temp = getValueAsObject(vr, elementLength);
          int vm = 1;
          if(temp instanceof Collection) vm = ((Collection)temp).size();
          if(temp instanceof String) if(temp.equals("")) temp = null;
          DICOMHashtable.put(Integer.toHexString(tag), new DICOMElement(tag, vr, vm, elementLength, temp));
        }
//System.out.println(" "  + temp);
        ...
        switch(tag) {
          case TRANSFER_SYNTAX_UID:
            ...
            if (transferSyntaxUID.indexOf("1.2.4") > -1 || transferSyntaxUID.indexOf("1.2.5") > -1) throw new IOException("Cannot open compressed DICOM images.\n Transfer Syntax UID = " + transferSyntaxUID);
            break;
          case NUM_FRAMES:
            nImages = Integer.parseInt(temp.toString().replace('+',' ').trim());
            break;
          case ROWS:
          ...
          case RED_PALETTE_COLOR_LOOKUP_TABLE_DESCRIPTOR:
            ...
            if(redLUTsize == 0) redLUTsize = 65536;
            if(pixelRepresentation == 1) redLUToffset = ((Short)temp).shortValue(); else redLUToffset = ((Integer)temp).intValue();
            redBits = ((Integer)temp).intValue();
            break;
          case GREEN_PALETTE_COLOR_LOOKUP_TABLE_DESCRIPTOR:
            greenLUTsize = ((Integer)temp).intValue();
            if(greenLUTsize == 0) greenLUTsize = 65536;
            if(pixelRepresentation == 1) greenLUToffset = ((Short)temp).shortValue(); else greenLUToffset = ((Integer)temp).intValue();
            greenBits = ((Integer)temp).intValue();
            break;
          case BLUE_PALETTE_COLOR_LOOKUP_TABLE_DESCRIPTOR:
            blueLUTsize = ((Integer)temp).intValue();
            if(blueLUTsize == 0) blueLUTsize = 65536;
            if(pixelRepresentation == 1) blueLUToffset = ((Short)temp).shortValue(); else blueLUToffset = ((Integer)temp).intValue();
            blueBits = ((Integer)temp).intValue();
            break;
          case RED_PALETTE_COLOR_LOOKUP_TABLE_DATA:
            lutSize = redLUTsize;
            if(lutSize > greenLUTsize) lutSize = greenLUTsize;
            if(lutSize > blueLUTsize) lutSize = blueLUTsize;
            redLUT = new byte[256];
            alphaLUT = new byte[256];
            for(int i = 0; i < alphaLUT.length; i++) alphaLUT[i] = (byte)127;
            if(redBits == 16) {
              for(int i = redLUToffset; i < lutSize; i++) redLUT[i] = (byte)(((Integer)temp).intValue() / 256.0);
            } else {
              for(int i = redLUToffset; i < lutSize; i++) redLUT[i] = (byte)((Short)temp).shortValue();
            }
            for(int i = lutSize; i < 255; i++) redLUT[i] = (byte)0;
            for(int i = 0; i < redLUToffset; i++) redLUT[i] = redLUT[0];
            break;
          case GREEN_PALETTE_COLOR_LOOKUP_TABLE_DATA:
            ...
            if(greenBits == 16) {
              for(int i = greenLUToffset; i < lutSize; i++) greenLUT[i] = (byte)(((Integer)temp).intValue() / 256.0);
            } else {
              for(int i = greenLUToffset; i < lutSize; i++) greenLUT[i] = (byte)((Short)temp).shortValue();
            }
            for(int i = lutSize; i < 255; i++) greenLUT[i] = (byte)0;
            for(int i = 0; i < greenLUToffset; i++) greenLUT[i] = greenLUT[0];
            break;
          case BLUE_PALETTE_COLOR_LOOKUP_TABLE_DATA:
            ...
            if(blueBits == 16) {
              for(int i = blueLUToffset; i < lutSize; i++) blueLUT[i] = (byte)(((Integer)temp).intValue() / 256.0);
            } else {
              for(int i = blueLUToffset; i < lutSize; i++) blueLUT[i] = (byte)((Short)temp).shortValue();
            }
            for(int i = lutSize; i < 255; i++) blueLUT[i] = (byte)0;
            for(int i = 0; i < blueLUToffset; i++) blueLUT[i] = blueLUT[0];
            break;
  ...
  private long getLength(int tag) throws IOException {
    ...
    if(implicit) {
      ...
      if(temp != null) vr = (temp.charAt(0)<<8) + temp.charAt(1);
      else vr = OX;
      ...
      case OX:
        ...
        break;
      default:
        if(bo == ByteOrder.BIG_ENDIAN) length = ((b2<<8) + b3); else length = ((b3<<8) + b2);
        break;
      }
    }
  ...
  private Object getValueAsObject(int vr, long elementLength) throws IOException {
    Object value = null;
    ...
    case UT:
      ...
      if(st.countTokens() > 1) {
        Vector v = new Vector();
        while (st.hasMoreTokens()) v.add(((String)st.nextToken()).trim());
        value = v;
      }
      break;
    case AT:
    ...
    case FL:
      if(elementLength != 4) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Float(stream.readFloat()));
        value = v;
      } else {
    ...
    case FD:
      if(elementLength != 8) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Float(stream.readDouble()));
        value = v;
      } else {
    ...
    case SL:
      if(elementLength != 4) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Integer(stream.readInt()));
        value = v;
      } else {
    ...
    case SS:
      if(elementLength != 2) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Short(stream.readShort()));
        value = v;
      } else {
    ...
    case UL:
      if(elementLength != 4) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Long(stream.readInt()&0xffffffff));
        value = v;
      } else {
    ...
    case US:
      if(elementLength != 2) {
        Vector v = new Vector();
        while((pos + elementLength) != stream.getStreamPosition()) v.add(new Integer(stream.readUnsignedShort()&0xffff));
        value = v;
      } else {
  ...
  private void getSQvals(ImageInputStream i, long l) throws IOException {
    ...
    do {
      ...
      long elementLength = getLength(tag);
      int vm = 1;
//System.out.println("  tag: " + Integer.toHexString(tag) + " length: " + elementLength + " vr: " + Integer.toHexString(vr));
      if(vr == SQ) {
        DICOMHashtable.put(Integer.toHexString(tag), new DICOMElement(tag, vr, vm, elementLength, ""));
        getSQvals(i, elementLength);
      } else if(tag == 0xfffee000 || tag == 0xfffee00d || tag == 0xfffee0dd) {
        DICOMHashtable.put(Integer.toHexString(tag), new DICOMElement(tag, vr, vm, elementLength, ""));
        if(tag == 0xfffee0dd) stop = true;
      } else {
...
//System.out.println("   temp: " + temp);
      ...
      if(l > -1 && (now - pos) >= l) stop = true;
//System.out.println("DFSDFDSFSDFSD: " + l + " now-pos: " + (now - pos));
    } while(stop == false);
  }

  private String getValueAsString(int vr, long elementLength) throws IOException {
    String value;
    switch (vr) {
    case AE:
    case AS:
  ...
  public Iterator getImageTypes(int imageIndex) throws IIOException {
    ...
    if(pixelRepresentation == 0) isSigned = false; else isSigned = true;
    ...
    if(photometricInterpretation.equalsIgnoreCase("MONOCHROME2") || photometricInterpretation.equalsIgnoreCase("MONOCHROME1")) {
      if(bitsAllocated <= 8) {
        l.add(ImageTypeSpecifier.createGrayscale(8, dataType, isSigned));
      } else {
        l.add(ImageTypeSpecifier.createGrayscale(16, dataType, isSigned));
      }
    } else if(photometricInterpretation.equalsIgnoreCase("PALETTE COLOR")) {
      l.add(ImageTypeSpecifier.createIndexed(redLUT, greenLUT, blueLUT, alphaLUT, 8, dataType));
    } else if(photometricInterpretation.equalsIgnoreCase("RGB")) {
      int[] bandOffsets = new int[samplesPerPixel];
      for(int i = 0; i < samplesPerPixel; i++) bandOffsets[i] = i;
      if(samplesPerPixel > 3) {
        l.add(ImageTypeSpecifier.createInterleaved(ColorSpace.getInstance(ColorSpace.CS_sRGB), bandOffsets, dataType, true, false));
      } else {
        l.add(ImageTypeSpecifier.createInterleaved(ColorSpace.getInstance(ColorSpace.CS_sRGB), bandOffsets, dataType, false, false));
      }
    } else {
      if(bitsAllocated <= 8) {
        l.add(ImageTypeSpecifier.createGrayscale(8, dataType, isSigned));
  ...
  public BufferedImage read(int imageIndex, ImageReadParam param) throws IIOException {
    ...
    try {
      ...
      for(int i = 0; i < inputBands; i++) bandOffsets[i] = i;
      ...
      if(destinationBands != null) imRas = imRas.createWritableChild(0, 0, imRas.getWidth(), imRas.getHeight(), 0, 0, destinationBands);
      ...
      if(dataType == DataBuffer.TYPE_BYTE) {
        ...
        //else
        //  for(int i = 0; i < samplesPerRow * height; i++) byteData[i] = (byte)(stream.readBits(bitsStored)&0x00000000ffffffff);
        ...
        //else
        //  for(int i = 0; i < samplesPerRow * height; i++) shortData[i] = (short)(stream.readBits(bitsStored)&0x00000000ffffffff);
        ...
        //else
        //  for(int i = 0; i < samplesPerRow * height; i++) intData[i] = (int)(stream.readBits(bitsStored)&0x00000000ffffffff);
  ...
  public Hashtable getDICOMHashtable() throws Exception {
    if(DICOMHashtable != null) return DICOMHashtable;
    readHeader();
  ...
  public void readMetadata() throws IIOException {
    ...
    try {
      ...
      int tag;
      long elementLength;
      while(stream.getStreamPosition() < imageStartPosition - 1 && (tag = getNextTag()) != 0x7FE00010) {
